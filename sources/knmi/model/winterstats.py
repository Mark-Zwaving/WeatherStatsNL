...
class Stats:
    ...
    def __init__(self, station, data ):
        ...
        self.npl_tn_lt_0   = stats.terms_days( data, 'TN', '<',   0 )
        self.npl_tn_lt__5  = stats.terms_days( data, 'TN', '<',  -5 )
        self.npl_tn_lt__10 = stats.terms_days( data, 'TN', '<', -10 )
        self.npl_tn_lt__15 = stats.terms_days( data, 'TN', '<', -15 )
        self.npl_tn_lt__20 = stats.terms_days( data, 'TN', '<', -20 )
        self.npl_hellmann  = self.npl_tg_lt_0

        # Counts days
        self.cnt_tx_lt_0   = self.npl_tx_lt_0.size
        self.cnt_tg_lt_0   = self.npl_tg_lt_0.size
        self.cnt_tn_lt_0   = self.npl_tn_lt_0.size
        self.cnt_tn_lt_0   = self.npl_tn_lt_0.size
        self.cnt_tn_lt__5  = self.npl_tn_lt__5.size
        self.cnt_tn_lt__10 = self.npl_tn_lt__10.size
        self.cnt_tn_lt__15 = self.npl_tn_lt__15.size
        self.cnt_tn_lt__20 = self.npl_tn_lt__20.size

        # Sum
        self.sum_hellmann  = abs(sum(self.npl_tg_lt_0, 'TG'))
...
def calculate( stations, sd, ed, name=False, type='html' ):
    ...
    for s in winter:
        ...
        if type == 'html':
            ...
            content += f'<td> {s.tg_gem} </td>'
            content += f'<td> {s.hellmann} </td>'
            content += f'<td> {s.tx_min} </td>'
            content += f'<td> {s.tg_min} </td>'
            content += f'<td> {s.tn_min} </td>'
            content += f'<td> {s.cnt_tx_lt_0} </td>'
            content += f'<td> {s.cnt_tn_lt_0} </td>'
            content += f'<td> {s.cnt_tg_lt_0} </td>'
            content += f'<td> {s.cnt_tn_lt__5} </td>'
            content += f'<td> {s.cnt_tn_lt__10} </td>'
            content += f'<td> {s.cnt_tn_lt__15} </td>'
            content += f'<td> {s.cnt_tn_lt__20} </td>'
            content += f'</tr>'

        elif type in ['txt','cmd']:
            content += f'{s.station.place:<23} {s.station.province:17} '
            content += f'{s.station.period:18} {s.tg_gem:7} {s.sum_hellmann:^8} '
            content += f'{s.tx_min:<7} {s.tg_min:<7} {s.tn_min:<7} {s.cnt_tx_lt_0:^5} '
            content += f'{s.cnt_tg_lt_0:^5} {s.cnt_tn_lt_0:^5} {s.cnt_tn_lt__5:^6} '
            content += f'{s.cnt_tn_lt__10:^6} {s.cnt_tn_lt__15:^6} {s.cnt_tn_lt__20:^6} \n'
