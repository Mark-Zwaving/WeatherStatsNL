import ask, config as cfg, fn, stats as st, zomerstats as zs, write as wr, fn_html as h, datum as d
...
def gen_calc_heat_waves(lijst_station, start_datum, eind_datum, type, name):
    ...
    if not name: file_name = fn.mk_path(path, f'{periode}-heatwaves.{type}') # Bepaal naam
    else: file_name = fn.mk_path(path, f'{name}.{type}') # Bepaal naam
    ...
    if heatwave_list:
        ...
        # TODO TODO TODO
            ...
            content = h.pagina(titel, h.style_winterstats_table(), content)
