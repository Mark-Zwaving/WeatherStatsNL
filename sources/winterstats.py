import write as wr, fn, stats as st, config as cfg, fn_html as h, datum
...
def alg_winterstats(lijst_stations, datum_start, datum_eind, type):
    ...
    file_name = fn.mk_path(path, f'{periode}-winterstatistieken.{type}') # Bepaal naam
    ...
    for station in lijst_stations:
        ...
        if not l: # Geen data gevonden, naar de volgende
            ...
            print(f"Berekenen statistieken station {station.wmo}, {station.plaats}")
    ...
    print(f"Maken {type}-betand met de statistieken")
    ...
    if type == 'txt':
        ...
        titel = 'winterstatistieken ' + periode
        table  = f'''
            <table>
                <thead>
                <tr> <th colspan="15"> {titel} </th> </tr>
                <tr>
                    <th> plaats </th>
                    <th> provincie </th>
                    <th> periode </th>
        ...
        content = h.pagina(titel, h.style_winterstats_table(), table)
