...
# See Class ColdwaveStats: below for the possible entities to put constraints on
def show ( coldwave ):
    ...
    if constraint:
        ...
        if  coldwave.day_count >= 6 and \
            coldwave.tn_lt_10['value'] >= 5:
            show = True
...
class ColdwaveStats:
    '''Class stores en calculates statistics in coldwaves'''
    '''Volgens het KNMI is een koudegolf een aaneengesloten periode van minstens 5 ijsdagen
       (etmaal waarbij de temperatuur niet boven het vriespunt komt), waarvan op 3 dagen de
       minimumtemperatuur lager is dan -10,0 graden (strenge vorst).'''
    def __init__(self, station, etm_l ):
        ...
        self.tx_max      =  st.min(etm_l,'TX')  # Dictionary { 'value', 'list_all_days' }
        self.tg_max      =  st.min(etm_l,'TG')  # Dictionary { 'value', 'list_all_days' }
        self.tn_max      =  st.min(etm_l,'TN')  # Dictionary { 'value', 'list_all_days' }
        ...
        self.tn_lt_5     =  st.cnt(etm_l,'TX','>=',300)  # Dictionary { 'value', 'list_all_days' }
        self.tn_lt_10    =  st.cnt(etm_l,'TX','>=',300)  # Dictionary { 'value', 'list_all_days' }
        self.tn_lt_15    =  st.cnt(etm_l,'TX','>=',350)  # Dictionary { 'value', 'list_all_days' }
        self.tn_lt_20    =  st.cnt(etm_l,'TX','>=',400)  # Dictionary { 'value', 'list_all_days' }
        self.sq_sum      =  st.sum(etm_l,'SQ') # Dictionary { 'value', 'list_all_days' }
        self.rh_sum      =  st.sum(etm_l,'RH') # Dictionary { 'value', 'list_all_days' }

def sort_coldwave_list(coldwaves, pm = '+'):
    sorted = []
    while coldwaves: # Check alle hittegolven
        days_max = 0
        heat_max = cfg.min_value_geg
        key = 0

        # Doorloop de hele lijst en haal de key van maximum waarde uit de lijst
        for ndx in range(len(coldwaves)):
            days_act = coldwaves[ndx].day_count
            heat_act = coldwaves[ndx].hellmann_sum_tot
...
def alg_coldwaves(lijst_station, ymd_s, ymd_e, type, name):
    '''Main function calculating heatwaves'''
    coldwave_lists, coldwaves, periode, path, content = [] ,[], f'{ymd_s}-{ymd_e}', '', ''
    bronvermelding = cfg.lijst_stations[0].notification
    max_rows = cfg.html_popup_table_max_rows

    if not name: name = f'coldwaves-{periode}-{d.act_datetime_unique()}'
    title = f'Coldwaves {periode}'
    filename = ''

    if type == 'html':
        name = f'{name}.html'
        path = cfg.dir_html_coldwaves
    if type == 'txt':
        name = f'{name}.txt'
        path = cfg.dir_text_coldwaves
    ...
    for station in lijst_station:
        ...
        if not l:
            ...
            print(f"Calculate coldwave statistics for station: {station.wmo} {station.plaats}")
            # Check for heatwaves
